...
```{r, eval = FALSE}
devtools::install_github("ebailey78/rucl")
library(rucl)
```
...
![openair](images/plume.png)

- The `openair` package was created by Dr. David Carslaw at King's College London.  The goals of the `openair` project are to create tools in R that use the wealth of air pollution data that is publicly available andmake it easy to carry out  sophisticated analyses quickly and in a reproducible way. 

- This is done by quickly summarizing and visualizing of air pollution data through wind and pollution roses, trend analyses and other helpful tools.

## Summary plot
- The first thing we want to do is a quick summary plot. 
- The summary plot shows time-series data and makes it easy to view missing data, as well.
- The summary plot also displays basic summary stats such as mean, median and 95th percentile.
...
- Could be especially useful for a modeler or analyst who wants to look at many years of wind data for comparison.

## Windrose

```{r}
windRose(mydata, type="year", paddle=F)
```
...
# Create a boxplot of grouped by the new Site.ID field
...
- We will use `raqdm` to retrieve VOC data from the Gary IITRI monitor.  
```{r,warning=FALSE, message=FALSE}
library(raqdm)
```
...
- The Gary IITRI station is a Photochemical Assessment Monitoring Station (PAMs).  We will query this data from the Gary IITRI site using the following request:
```{r, warning=FALSE, eval=FALSE}
pams2 <- getAQDMdata(user = "me@email.com", pw = "abc123", state = "18", county = "089", site = "0022", pc = "PAMS", param="", format = "DMCSV", bdate = "20140601", edate = "20140602", frmonly = "n", synchronous = TRUE)
```

```{r, warning=FALSE, eval=FALSE, echo=FALSE}
pams2 <- getAQDMdata(user = "kdfrost14@gmail.com", pw = "carmelkit41", state = "18", county = "089", site = "0022", pc = "PAMS", param="", format = "DMCSV", bdate = "20140601", edate = "20140602", frmonly = "n", synchronous = TRUE)
```
...
- I already have a large file from which I did an asynchronous pull straight from the Data Mart website.  This is one year of PAMS data from the Gary IITRI monitor.

- Since the data mart files come with an "End of File" marker, we will tell R to ignore the last two lines of data.
```{r,eval=FALSE}
pams_len <- length(readLines("PAMS_IITRI.txt"))  ##Gives us the number of rows in the data file without using memory to read in the whole file.
pams <- read.table("PAMS_IITRI.txt", header=T, sep=",", nrows=pams_len-2)  ##Reads in the file and ignores the last 2 lines.
```

```{r,echo=FALSE}
pams <- readRDS(file="data/PAMS_IITRI.rda")
```
...
- Then we will select only the columns of interest to us using the `select()` function from `dplyr`.  For this analysis, we only need Date, Parameter, and Sample Measurement.

```{r}
IITRI_sub <- select(IITRI, Date.GMT,  X24.Hour.GMT, AQS.Parameter.Desc, Sample.Measurement)
```
...
```{r,echo=FALSE}
library(openair)
gary <- import("data/Gary_openair.csv",sep=",",date="Date.GMT", time="X24.Hour.GMT" , date.format="%Y-%m-%d", time.format="%H:%M", ws="Wind Speed - Resultant", wd="Wind Direction - Resultant")
```
...
## Percentile Rose
- A percentile rose plot can help you see the distribution of concentrations by wind direction and specify high concentrations.
...
## Percentile Rose
```{r}
percentileRose(gary, pollutant="Benzene", percentile = 95, method="cpf", smooth=TRUE, main="95th Percentile Concentrations of Benzene & Their Associated Wind Direction")
```

## Time-series plots

```{r}
timePlot(gary,pollutant=c("Benzene","Toluene","ws"))
```
...
##Scatterplot with wind direction as the 'z' value

```{r}
scatterPlot(selectByDate(gary, month = 8),x = "date", y = "Benzene", z = "wd")
```

##Trend Levels (Data Heatmaps)

```{r,eval=FALSE}
trendLevel(selectByDate(gary, year=2014), pollutant = "Benzene", border = "white", statistic = "max", breaks = c(0, 10, 20, 200), labels = c("acceptable", "near SL", "above SL"), cols = c("forestgreen", "yellow", "red"),main = "Trends in Benzene Concentrations")
```

## Trend Levels 
```{r,echo=FALSE}
trendLevel(mydata, pollutant = "no2", border = "white", statistic = "max", breaks = c(0, 50, 100, 500), labels = c("low", "medium", "high"), cols = c("forestgreen", "yellow", "red"))
```
